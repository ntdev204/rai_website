"""WebSocket router for robot telemetry and adaptive runtime AI metrics.

Pushes merged telemetry (Pi robot state + adaptive runtime AI metrics) to all connected clients.
"""
...
async def ws_telemetry(websocket: WebSocket, token: str = Query(default="")):
    ...
    try:
        while True:
            ...
            # Fetch AI metrics from the adaptive runtime (non-blocking, cached TTL)
            adaptive_metrics = await adaptive_proxy.get_metrics()
